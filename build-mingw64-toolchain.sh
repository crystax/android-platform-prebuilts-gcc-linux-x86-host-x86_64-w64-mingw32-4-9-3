...
Valid options:
  ...
  --isl-version=<version>      Select libisl version [$ISL_VERSION]
  --cloog-version=<version>    Select libcloog version [$CLOOG_VERSION]
...
download_package http://isl.gforge.inria.fr/isl-$ISL_VERSION.tar.bz2
download_package ftp://gcc.gnu.org/pub/gcc/infrastructure/cloog-$CLOOG_VERSION.tar.gz
...
if [ "$MINGW_W64_VERSION_NO_REV" = "svn" ];  then
    ...
    MINGW_W64_SRC=$SRC_DIR/mingw-w64-v$MINGW_W64_VERSION
fi

if [ ! -d $MINGW_W64_SRC ]; then
    if [ -n "$MINGW_W64_REVISION" ]; then
        ...
        fail_panic "Can't fetch mingw-w64 sources"
    else
        #download_package http://downloads.sourceforge.net/project/mingw-w64/mingw-w64/mingw-w64-release/mingw-w64-v$MINGW_W64_VERSION.tar.bz2?r=&ts=1437085954&use_mirror=ufpr
        download_package http://downloads.sourceforge.net/project/mingw-w64/mingw-w64/mingw-w64-release/mingw-w64-v$MINGW_W64_VERSION.tar.bz2
        fail_panic "Can't fetch mingw-w64 sources"
    fi
...
build_host_package mpfr-$MPFR_VERSION $BASE_HOST_OPTIONS --with-gmp=$INSTALL_DIR

build_host_package mpc-$MPC_VERSION $BASE_HOST_OPTIONS --with-gmp=$INSTALL_DIR --with-mpfr=$INSTALL_DIR

build_host_package isl-$ISL_VERSION $BASE_HOST_OPTIONS --with-gmp-prefix=$INSTALL_DIR

build_host_package cloog-$CLOOG_VERSION $BASE_HOST_OPTIONS --with-gmp-prefix=$INSTALL_DIR --with-isl-prefix=$INSTALL_DIR
...
var_append BASE_HOST_OPTIONS "--with-isl=$INSTALL_DIR"
...
build_mingw_libraries()
{
    ...
    for BITS in 32 64; do
        if [ ! -f "$STAMP_DIR/$PKGNAME-$BITS" ]; then
            (
                mkdir -p $BUILD_DIR/$PKGNAME-$BITS &&
                cd $BUILD_DIR/$PKGNAME-$BITS &&
                export PATH=$INSTALL_DIR/bin:$PATH
                export CFLAGS="-m$BITS"

                log "$PKGNAME-$BITS: Configuring" &&
                run $MINGW_W64_SRC/mingw-w64-libraries/winpthreads/configure \
                    --prefix=$PREFIX_FOR_TARGET \
                    --libdir=$PREFIX_FOR_TARGET/lib$BITS \
                    --host=$TARGET_TAG \
                    --with-sysroot=$INSTALL_DIR \
                    --disable-shared
                fail_panic "Can't configure $PKGNAME-$BITS"

                run make -j$JOBS
                fail_panic "Can't build $PKGNAME-$BITS"

                log "$PKGNAME-$BITS: Installing" &&
                run make install
                fail_panic "Can't install $PKGNAME-$BITS"
            ) || exit 1
        fi
        touch $STAMP_DIR/$PKGNAME-$BITS
    done
}
...
build_mingw_libraries mingw-w64-libraries
